...
create or replace function public.slugify_event_name(input text)
returns text language sql immutable set search_path = '' as $$
  select trim(both '-' from regexp_replace(regexp_replace(lower(public.unaccent(input)), '[^a-z0-9]+', '-', 'g'), '-+', '-', 'g'))
$$;

create or replace function public.admin_create_event(
  p_name text, p_slug text, p_location text, p_asset_path text, p_timezone text,
  p_start_local timestamp, p_end_local timestamp, p_lock_local timestamp,
  p_visible_until_local timestamp, p_status public.event_status default 'draft'
)
...
begin
  ...
  insert into public.events(name, slug, location, asset_path, timezone, start_at, end_at, lock_at, visible_until, status, slug_locked_at, created_by_profile_id, updated_by_profile_id)
  values(btrim(p_name), normalized_slug, btrim(p_location), nullif(btrim(p_asset_path), ''), p_timezone, start_value, end_value, lock_value, visible_value, p_status,
  ...
  values(actor_id, 'event.created', 'event', new_event.event_id, new_event.event_id, jsonb_build_object('name', new_event.name, 'status', new_event.status));
  return next new_event;
end $$;

create or replace function public.manage_get_event(p_event_id uuid)
...
begin
  ...
  return query select * from public.events where event_id = p_event_id;
end $$;

create or replace function public.manage_update_event(
  p_event_id uuid, p_name text, p_slug text, p_location text, p_asset_path text, p_timezone text,
  p_start_local timestamp, p_end_local timestamp, p_lock_local timestamp,
  p_visible_until_local timestamp, p_status public.event_status
)
returns setof public.events language plpgsql security definer set search_path = '' as $$
...
begin
  ...
  select profile_id into actor_id from public.profiles where auth_user_id = auth.uid();
  update public.events set name=btrim(p_name), slug=normalized_slug, location=btrim(p_location), asset_path=nullif(btrim(p_asset_path), ''), timezone=p_timezone,
  ...
  insert into public.audit_log(actor_profile_id, action, entity_type, entity_id, event_id, change_information)
  values(actor_id, 'event.updated', 'event', p_event_id, p_event_id, jsonb_build_object('name_from', old_event.name, 'name_to', updated_event.name, 'status_from', old_event.status, 'status_to', updated_event.status));
  return next updated_event;
end $$;

grant execute on function public.admin_create_event(text,text,text,text,text,timestamp,timestamp,timestamp,timestamp,public.event_status) to authenticated;
grant execute on function public.manage_get_event(uuid) to authenticated;
grant execute on function public.manage_update_event(uuid,text,text,text,text,text,timestamp,timestamp,timestamp,timestamp,public.event_status) to authenticated;
