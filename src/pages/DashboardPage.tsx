...
type DashboardStats={event_count:number;active_event_count:number;people_count:number;editor_count:number;report_count:number}

export function DashboardPage() {
  const { profile } = useAuth()
  const [stats,setStats]=useState<DashboardStats|null>(null)
  const [error,setError]=useState('')
  useEffect(()=>{void supabase.rpc('manage_dashboard_stats').then(({data,error})=>{if(error)setError(error.message);else setStats(data as DashboardStats)})},[])
  const cards = [
    { label:'Events',value:stats?.event_count,detail:stats?`${stats.active_event_count} active`:'Loading…',icon:CalendarDays,to:'/manage/events',tone:'bg-blue-50 text-blue-700' },
    { label:'People',value:stats?.people_count,detail:stats?`${stats.editor_count} Editor${stats.editor_count===1?'':'s'}`:'Loading…',icon:Users,to:'/manage/users',tone:'bg-orange-50 text-[#d94722]',admin:true },
    { label:'Reports',value:stats?.report_count,detail:stats?(stats.report_count?'Need moderation':'No items need review'):'Loading…',icon:MessageSquareWarning,to:'/manage/moderation',tone:'bg-emerald-50 text-emerald-700' },
  ]
  return <>
    ...
    {error&&<p className="mt-6 rounded-xl bg-red-50 p-3 text-sm font-medium text-red-700">Unable to load dashboard totals: {error}</p>}
