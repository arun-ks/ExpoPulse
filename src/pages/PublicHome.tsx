...
type PublicEvent = { event_id:string; name:string; slug:string; location:string; website_url:string|null; description:string|null; asset_path:string|null; start_at:string }
...
export function PublicHome() {
  ...
  useEffect(()=>{ void (async()=>{ const {data,error}=await supabase.rpc('public_list_visible_events_v2'); if(error)setError('Events could not be loaded. Please try again.'); else setEvents((data??[]) as PublicEvent[]); setLoading(false) })() },[])
  return <main className="min-h-screen bg-[#f4f7f9]">
    ...
    <section className="mx-auto max-w-7xl px-5 py-14 md:py-20"><span className="text-sm font-black uppercase tracking-[.2em] text-[#ef5b32]">Discover what matters</span><h1 className="mt-4 max-w-3xl text-4xl font-black tracking-tight text-[#0b2940] md:text-6xl">Find the people and ideas shaping your event.</h1><p className="mt-5 max-w-2xl text-lg leading-8 text-slate-600">Browse exhibitors, locate booths and share thoughtful feedback—all from your phone.</p>
      ...
      <div className="mt-5 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{events.map(event=><article key={event.event_id} className="card group flex flex-col overflow-hidden transition hover:-translate-y-1 hover:shadow-lg"><Link to={`/e/${event.slug}`} className="flex flex-1 flex-col">{event.asset_path?<img className="h-40 w-full object-cover" src={event.asset_path} alt=""/>:<div className="h-28 bg-gradient-to-br from-[#0b2940] to-[#28516d]"/>}<div className="flex flex-1 flex-col p-5"><h3 className="text-xl font-black text-slate-900">{event.name}</h3>{event.description&&<p className="mt-3 text-sm leading-6 text-slate-600">{event.description}</p>}<p className="mt-3 flex gap-2 text-sm text-slate-600"><CalendarDays className="shrink-0" size={17}/>{new Date(event.start_at).toLocaleDateString(undefined,{dateStyle:'medium'})}</p><p className="mt-2 flex gap-2 text-sm text-slate-600"><MapPin className="shrink-0" size={17}/>{event.location}</p><span className="mt-5 inline-flex items-center gap-2 font-bold text-[#ef5b32]">Explore exhibitors <ArrowRight size={17}/></span></div></Link>{event.website_url&&<a className="mx-5 mb-5 inline-flex items-center gap-2 self-start text-sm font-bold text-[#d94722] hover:underline" href={event.website_url} target="_blank" rel="noreferrer">Visit event website<ExternalLink size={15}/></a>}</article>)}</div>
    </section>
  </main>
}
