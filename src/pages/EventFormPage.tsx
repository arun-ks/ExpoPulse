...
type FormState = { name:string; slug:string; location:string; website:string; description:string; assetPath:string; timezone:string; start:string; end:string; lock:string; visibleUntil:string; status:'draft'|'active'|'archived' }
...
export function EventFormPage({ mode }: { mode: 'create' | 'edit' }) {
  ...
  const [form, setForm] = useState<FormState>({ name:'', slug:'', location:'', website:'', description:'', assetPath:'', timezone:Intl.DateTimeFormat().resolvedOptions().timeZone || 'Asia/Singapore', start:defaults.start, end:defaults.end, lock:defaults.lock, visibleUntil:defaults.visibleUntil, status:'draft' })
  ...
  useEffect(() => {
    ...
    void supabase.rpc('manage_get_event', { p_event_id: eventId }).then(({ data, error: loadError }) => {
      ...
      setForm({ name:item.name, slug:item.slug, location:item.location, website:item.website_url ?? '', description:item.description ?? '', assetPath:item.asset_path ?? '', timezone:item.timezone, start:isoToEventLocal(item.start_at,item.timezone), end:isoToEventLocal(item.end_at,item.timezone), lock:isoToEventLocal(item.lock_at,item.timezone), visibleUntil:isoToEventLocal(item.visible_until,item.timezone), status:item.status })
  ...
  const submit = async (event: FormEvent) => {
    ...
    const common = { p_name:form.name, p_slug:form.slug, p_location:form.location, p_website_url:form.website, p_description:form.description, p_asset_path:form.assetPath, p_timezone:form.timezone, p_start_local:form.start, p_end_local:form.end, p_lock_local:form.lock, p_visible_until_local:form.visibleUntil, p_status:form.status }
    const result = mode === 'create' ? await supabase.rpc('admin_create_event_v2', common) : await supabase.rpc('manage_update_event_v2', { p_event_id:eventId, ...common })
    ...
    <form onSubmit={submit} className="mt-7 grid gap-6 xl:grid-cols-[1fr_360px]">
      <div className="space-y-6">
        <section className="card p-6"><div className="flex items-center gap-3"><div className="grid size-10 place-items-center rounded-xl bg-orange-50 text-[#ef5b32]"><CalendarCheck size={20}/></div><div><h2 className="font-bold text-slate-900">Event identity</h2><p className="text-sm text-slate-500">The information visitors will recognize.</p></div></div><div className="mt-6 grid gap-5 sm:grid-cols-2">
          ...
          <label className="block sm:col-span-2"><span className="label">Website <span className="font-normal text-slate-400">(optional)</span></span><input className="field" type="url" placeholder="https://example.com" value={form.website} onChange={e=>update('website',e.target.value)} /></label>
          <label className="block sm:col-span-2"><span className="label">Description <span className="font-normal text-slate-400">(optional)</span></span><textarea className="field min-h-20" maxLength={200} placeholder="A short summary of the Event" value={form.description} onChange={e=>update('description',e.target.value)} /><span className="mt-1.5 block text-right text-xs text-slate-500">{form.description.length}/200</span></label>
